...
import type { FilterState, LookupData } from '@/types/graph';
...
const FilterPanel = ({ filters, lookupData, onChange, onApply, isLoading }: FilterPanelProps) => {
        ...
        <div className="mt-1 bg-card/95 backdrop-blur-sm border border-border rounded-lg p-3 space-y-3 max-h-[calc(100vh-120px)] overflow-y-auto">
          ...
          <div>
            ...
            >
              ...
              {lookupData.orders.map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              {lookupData.plants.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              {lookupData.materials.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              {lookupData.currencies.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          ...
          <div className="flex gap-2 pt-1">
            <Button size="sm" onClick={onApply} disabled={isLoading} className="flex-1 h-7 text-xs">
              Apply
            </Button>
